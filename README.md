...
```text
AOI или bbox → поиск Sentinel-сцен → выбор срока → GeoTIFF/COG asset → AOI crop → local CFAR detector → обзорный PNG → crop судов → GeoJSON/CSV/Parquet/report.json → Telegram
```
...
- Shoreline/land mask и подавление береговых ложных срабатываний.
...
`report.json` содержит параметры detector-а, raster key, product id, acquisition time, число детекций, paths crop-файлов и provenance.

## Текущий план реализации

См. `docs/IMPLEMENTATION_PLAN.md`.

Ближайший следующий этап: shoreline/land mask + более строгий local CFAR, затем wake association и AIS track rendering.
