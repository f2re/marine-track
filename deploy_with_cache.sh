#!/usr/bin/env bash
# Cache-aware deploy wrapper: provider deploy + one-time land mask + cleanup.
...
usage() {
  cat <<EOF
Deploy Marine Track with providers, one-time land-mask preparation and cache cleanup.

Usage:
  ./deploy_with_cache.sh [options]
...
or MARINE_TRACK_FORCE_UPDATE_LAND_MASK=1 is set. Cleanup retention is controlled
by MARINE_TRACK_*_RETENTION_DAYS variables in .env.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-restart) NO_RESTART=1; EXTRA_ARGS+=("$1"); shift ;;
    -h|--help) usage; exit 0 ;;
    *) EXTRA_ARGS+=("$1"); shift ;;
  esac
done

MARINE_TRACK_PROVIDER_PROFILE="$PROVIDER_PROFILE" "$SCRIPT_DIR/deploy_with_providers.sh" --no-restart "${EXTRA_ARGS[@]}"
...
echo "Cache-aware deployment complete: profile=$PROVIDER_PROFILE"
