...
- [x] `deploy_telegram_bot.sh` содержит provider preflight, Telegram token prompt/getMe, command registration, land mask once-on-deploy и cleanup.
- [x] Runtime-check учитывает `MARINE_TRACK_PROVIDER_PROFILE` и не валит core-only deployment.
- [x] Убран compatibility-layer: `BOT_TOKEN`, provider-profile `none`, legacy `last_bbox` schema.
...
- [x] `install_telegram_bot.sh` and `deploy_telegram_bot.sh` support current profiles `all|scene|aux|core`.
...
- [x] Compatibility aliases removed from runtime contract: `none` profile and `BOT_TOKEN` fallback.
...
### Этап 9. Упрощение кодовой базы

- [x] Убрать `BOT_TOKEN` fallback; использовать только `TELEGRAM_BOT_TOKEN`.
- [x] Убрать provider-profile alias `none`; оставить только `all|scene|aux|core`.
- [x] Убрать чтение/миграцию legacy `last_bbox`; оставить только `saved_bboxes`.
- [x] Вернуть provider preflight как текущий Python-модуль без сетевых запросов и без legacy alias.
- [x] Исправить sync `.env`: добавлять отсутствующие ключи по факту наличия ключа, а не по непустому значению.
- [ ] Удалить командный alias `/bboxlist`, оставить только `/areas`.
- [ ] Дочистить `.env.example` и `docs/PROVIDERS.md` от текстовых упоминаний `core/none`.
...
- 2026-07-06: provider dependencies вынесены в optional extras; install/deploy получили provider profiles; `.env` синхронизирует `MARINE_TRACK_PROVIDER_PROFILE`; runtime-check стал profile-aware и поддерживает core-only deployment без provider packages.
...
- 2026-07-07: начато упрощение кодовой базы: удалены `BOT_TOKEN`, profile alias `none`, legacy state schema `last_bbox`; provider preflight вынесен в текущий Python-модуль `marine_track.provider_preflight`; deploy снова запускает preflight и корректно синхронизирует пустые optional env keys без дублирования.
