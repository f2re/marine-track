...
- [x] Land mask подготавливается при cache-aware deploy один раз, если файла еще нет.
...
- [x] Cache-aware wrappers: `install_with_cache.sh`, `deploy_with_cache.sh`.
...
- [x] одноразовая сборка land mask при `deploy_with_cache.sh`, если mask-файла еще нет.
...
- [x] `deploy_prepare.py`: land mask once-on-deploy + cleanup on deploy.
- [x] `install_with_cache.sh` и `deploy_with_cache.sh`: cache-aware wrappers.
- [x] `marine-track cleanup-cache`: ручная очистка кешей и outputs.
- [ ] Расширить кеширование на обычные `/dates` и `/bboxdates`, а не только `/detectbbox`.
...
- 2026-07-06: реализован cache lifecycle: TTL-кеш scene search для `/detectbbox`, общий raster cache, once-on-deploy land mask через `deploy_prepare.py`, retention cleanup, cache-aware wrappers, CLI `cleanup-cache`, Telegram/report cache status.
